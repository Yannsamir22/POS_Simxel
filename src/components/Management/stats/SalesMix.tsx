...
const SEGMENTS = [
  { key: "Services", color: "#16a34a" },
  { key: "Products", color: "#0197f6" },
  { key: "Packages", color: "#0ed8e3" },
];
...
const SalesMix: React.FC<SalesMixProps> = ({
  ...
  const serviceTotal = (topServices ?? []).reduce((s, e) => s + e.revenue, 0);
  const productTotal = (topProducts ?? []).reduce((s, e) => s + e.revenue, 0);
  const packageTotal = (topPackages ?? []).reduce((s, e) => s + e.revenue, 0);
