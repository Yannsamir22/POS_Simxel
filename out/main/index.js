...
function setupUpdater(win) {
  ...
  main$1.autoUpdater.on("error", (error2) => {
    console.error("[Updater] Error: ", error2?.message ?? error2);
  });
  require$$1$1.ipcMain.on("install-update", () => {
    main$1.autoUpdater.quitAndInstall(
      false,
      // isSilent - show installer dialog on Windows
      true
      // isForceRunAfter - relaunch after install
    );
  });
  require$$1$1.ipcMain.on("check-for-updates", () => {
    main$1.autoUpdater.checkForUpdates().catch((error2) => {
      console.error("[Updater] Manual check failed: ", error2?.message);
    });
  });
  setTimeout(() => {
    main$1.autoUpdater.checkForUpdates().catch(
      (error2) => console.error("[Updater] Initial check failed: ", error2?.message)
    );
  }, 1e4);
}
const isDev = !require$$1$1.app.isPackaged;
const USER_DATA = require$$1$1.app.getPath("userData");
const RECEIPTS_DIR = path$m.join(USER_DATA, "receipts");
const EXPORTS_DIR = path$m.join(USER_DATA, "exports");
const DB_PATH = path$m.join(USER_DATA, "simxel.db");
[RECEIPTS_DIR, EXPORTS_DIR].forEach(
  (dir) => fs$j.mkdirSync(dir, { recursive: true })
);
let backendProcess = null;
function getBackendEntry() {
  if (isDev) {
    return path$m.resolve(__dirname, "../../../../Simxel/dist/server.js");
  }
  return path$m.join(process.resourcesPath, "backend", "server.js");
}
function startBackend() {
  const entry = getBackendEntry();
  if (!fs$j.existsSync(entry)) {
...
Please reinstall the application.`
  ...
  backendProcess = require$$1$2.spawn(process.execPath, [entry], {
    env: {
      ...process.env,
  ...
  backendProcess.on(
    "error",
    (err) => console.error("[Backend] Process error:", err)
  );
  backendProcess.on(
    "exit",
    (code, signal) => console.log(`[Backend] Exited — code=${code} signal=${signal}`)
  );
...
function createWindow() {
  mainWindow = new require$$1$1.BrowserWindow({
    ...
    minHeight: 640,
    title: "Simxel POS",
    backgroundColor: require$$1$1.nativeTheme.shouldUseDarkColors ? "#1d232a" : "#ffffff",
    webPreferences: {
  ...
  if (isDev) {
    ...
    mainWindow.loadFile(path$m.join(__dirname, "../../dist/index.html"));
  }
  mainWindow.once("ready-to-show", () => mainWindow?.show());
  mainWindow.on("closed", () => mainWindow = null);
}
require$$1$1.ipcMain.handle("get-receipts-dir", () => RECEIPTS_DIR);
require$$1$1.ipcMain.handle("get-exports-dir", () => EXPORTS_DIR);
require$$1$1.ipcMain.handle("open-receipts-folder", () => require$$1$1.shell.openPath(RECEIPTS_DIR));
require$$1$1.ipcMain.handle("open-exports-folder", () => require$$1$1.shell.openPath(EXPORTS_DIR));
require$$1$1.ipcMain.handle("open-file", async (_event, filePath) => {
  const candidates = [];
  if (path$m.isAbsolute(filePath)) {
    candidates.push(filePath);
  } else {
    candidates.push(
      path$m.join(USER_DATA, filePath),
      path$m.join(EXPORTS_DIR, filePath),
      path$m.join(RECEIPTS_DIR, filePath)
    );
  }
  const resolved = candidates.find((p) => fs$j.existsSync(p));
  if (!resolved) {
    return {
      ok: false,
...
${candidates.join("\n")}`
  ...
  const err = await require$$1$1.shell.openPath(resolved);
  return err ? { ok: false, error: err } : { ok: true };
});
require$$1$1.ipcMain.handle(
  "print-receipt",
  async (_event, filePath, silent = false) => {
    const resolved = path$m.isAbsolute(filePath) ? filePath : path$m.join(RECEIPTS_DIR, filePath);
    if (!fs$j.existsSync(resolved)) {
      return { ok: false, error: `Receipt not found: ${resolved}` };
    }
    return new Promise((resolve) => {
      const printWin = new require$$1$1.BrowserWindow({
        show: false,
        ...
        printWin.webContents.print(
          ...
          (success, reason) => {
            ...
            if (success) {
              ...
              require$$1$1.shell.openPath(resolved);
...
require$$1$1.ipcMain.handle("list-receipts", () => {
  try {
    return fs$j.readdirSync(RECEIPTS_DIR).filter((f) => f.endsWith(".pdf")).map((f) => ({
      name: f,
      path: path$m.join(RECEIPTS_DIR, f),
      mtime: fs$j.statSync(path$m.join(RECEIPTS_DIR, f)).mtime
    })).sort((a, b) => b.mtime.getTime() - a.mtime.getTime());
  } catch {
    return [];
  }
});
require$$1$1.ipcMain.handle("list-exports", () => {
  try {
    return fs$j.readdirSync(EXPORTS_DIR).filter((f) => f.match(/\.(xlsx|csv)$/i)).map((f) => ({
      name: f,
      path: path$m.join(EXPORTS_DIR, f),
      mtime: fs$j.statSync(path$m.join(EXPORTS_DIR, f)).mtime
    })).sort((a, b) => b.mtime.getTime() - a.mtime.getTime());
  } catch {
    return [];
  }
});
require$$1$1.ipcMain.handle("get-app-version", () => require$$1$1.app.getVersion());
...
require$$1$1.app.on("window-all-closed", () => {
  stopBackend();
  if (process.platform !== "darwin") require$$1$1.app.quit();
});
require$$1$1.app.on("before-quit", () => stopBackend());
